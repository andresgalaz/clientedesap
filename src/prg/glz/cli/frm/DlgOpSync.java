...
public class DlgOpSync extends JDialog {
    ...
    private String buildMsg(String cMsg, String cArch) {
        return "<html><div style=\"background-color:'" + Constante.STR_COLOR_FONDO + "';\">" + //
                "<font size='4' color='" + Constante.STR_COLOR_LABEL + "' face='Verdana'>" + cMsg + "</font><br>" + "<br>" + //
                "<font size='5' color='" + Constante.STR_COLOR_LABEL + "' face='Verdana'>" + cArch + "</font><br>" + //
                "</div><html>";
    }

    public int cofirmaUpdate(String cArch, boolean bDiffContenido) {
        Integer nAutoResp = getAutoRespuesta();
        if (nAutoResp != null)
            return respuesta = nAutoResp;

        if (bDiffContenido)
            lbMsg.setText( buildMsg( "El contenido de los archivos es diferente:", cArch ) );
        else
        ...
        pack();
        getContentPane().setBackground( Constante.COLOR_FONDO );
        setVisible( true );
        setAutoRespuesta();
        return respuesta;
    }
    ...
    public DlgOpSync(Frame owner, String title, boolean modal) {
        ...
        super.getContentPane().setBackground( Constante.COLOR_FONDO );
        ...
        messagePane.setBackground( Constante.COLOR_FONDO );
        ...
        // DlgOpSync.this.tpObjeto =
        // TTpFormObjeto.getTpFormObjetoByDescripcion(cbTpForm.getSelectedItem().toString());
        // }
        // });
        //
        // messagePane.add(cbTpForm);

        getContentPane().setBackground( Constante.COLOR_FONDO2 );
        getContentPane().add( messagePane );

        // Create a button
        JPanel buttonPane = new JPanel( new GridLayout( 3, 1 ) );
        buttonPane.setBorder( BorderFactory.createEmptyBorder( 8, 8, 8, 8 ) );
        buttonPane.setBackground( Constante.COLOR_FONDO );
    ...
    private Integer getAutoRespuesta() {
        ...
        // Actualiza timestamp de la respuesta, asi le da 5 segundos mas de vida
        mapTimeRepite.put( cTpDialogo, new Date().getTime() );
        return nRespuesta;
    }
